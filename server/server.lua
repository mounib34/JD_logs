...
function HidePlayerDetails(message, color, channel)
	if Config.AllLogs then
		PerformHttpRequest(Config.webhooks["all"], function(err, text, headers) if err ~= 204 then print('Warn: JD_logs webhook. Invalid "all" webhook') end end, 'POST', json.encode({
  	...
  	PerformHttpRequest(Config.webhooks[channel], function(err, text, headers) if err ~= 204 then print('Warn: JD_logs webhook. Invalid "'..channel..'" webhook') end end, 'POST', json.encode({
		username = Config.username, 
		embeds = {{
			["color"] = color, 
			["author"] = {
...
function PluginHidePlayerDetails(message, color, channel)
	...
	if Config.AllLogs then
  	...
  	PerformHttpRequest(Config.Plugins[channel].webhook, function(err, text, headers) if err ~= 204 then print('Warn: JD_logs webhook. Invalid "'..channel..'" webhook') end end, 'POST', json.encode({
		username = Config.username, 
		embeds = {{
			["color"] = _color, 
			["author"] = {
